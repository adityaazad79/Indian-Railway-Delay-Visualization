    ...
    with st.container():
        ...
        if source_station and destination_station and source_station != destination_station:
            ...
            if train_segments:
                ...
                # Add vertical dotted lines at 4-hour intervals
...
# =================== Page 1: Train Type Delay ===================
if menu == "Train Type Delay":
    st.title("🐝 Train Type vs Delay (Swarm Plot)")
    ...
    for train_type, filename in train_type_files.items():
        try:
            ...
            # Remove leading/trailing whitespaces and ensure consistent casing
            df.columns = df.columns.str.strip()
            df["Train_No"] = df["Train_No"].astype(str).str.strip()
            df["Delay"] = pd.to_numeric(df["Delay"], errors="coerce")

            # Get one average delay per train
    ...
    if delay_data:
        ...
        sns.stripplot(data=combined_df, x="Train_Type", y="Delay", palette="Set2", size=3, jitter=True, ax=ax)

        ax.set_title("Train Delay Distribution by Train Type (One Dot Per Train)")
